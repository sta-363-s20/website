...
1. Based on the help function, how many rows (n) and how many columns (p) does the `Auto` file have? What are the variables included in the data frame? Add your responses to your lab report. When you're done, commit your changes with the commit message "Added answer for Ex 1", and push.
...
Start with the `Auto` dataset and pipe it into the `mutate` function to create a new variable called `cylinders` that is a factor variable. Store the resulting filtered data frame as a new data frame called `auto_new`.
...
ggplot(data = auto_new, mapping = aes(x = cylinders, y = mpg)) +
  geom_boxplot()
```
...
5. Let's fit a model. This may be a new way to fit models, again the code is going to be provided, but see if you can follow along. We are going to fit a model predicting miles per gallon from the variables in the `Auto` data frame, excluding the `name` column. Copy the code below into your Rmd file. Try to explain what each line of the "recipe" code is doing.

First, let's set up how we will fit this model. We will use the `recipe()` function to dictate the model and the data. It may not seem clear now why these steps are helpful, but as the class goes on you will see how setting up these "recipies" can ultimately save a lot of time. Try to explain what each line of this code is doing. Finally, explain how the `weight` variable is related to `mpg`.
...
rec <- Auto %>%
  ...
  step_mutate(cylinders = as.factor(cylinders),
...
Let's prep this recipe and then fit the model. You don't need to explain this part, just copy this code to your Rmd file.
...
Here is the output from our model. How is the `weight` variable related to `mpg`?
...
*This is a good place to pause, commit changes with the commit message "Added answer for Ex 5", and push.*
